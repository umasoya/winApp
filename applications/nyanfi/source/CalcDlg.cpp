...
long double __fastcall TCalculator::EvalFunc(UnicodeString s)
{
	...
	try {
		...
		if (p>0) {
			...
			else if (ans==_LHUGE_VAL || !_finitel(ans) || _isnanl(ans)) {
	...
	int p)					//演算位置
{
	...
	try {
		...
		if (ans==_LHUGE_VAL || !_finitel(ans) || _isnanl(ans)) {
			ErrMsg = "異常値またはオーバーフロー";  Abort();
		}
	...
	bool use_cb)		//クリップボードを介して実行
{
	...
	exline = REPLACE_TI(exline, " XOR ", "＾");
	exline = REPLACE_TI(exline, " OR ", "|");
	exline = REPLACE_TI(exline, " GCD ", "Ｇ");
	exline = REPLACE_TI(exline, " LCM ", "Ｌ");
