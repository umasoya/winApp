		...
		if (tag.IsEmpty()) {
			...
			//リンク
				...
				if (ToMarkdown && !HrefStr.IsEmpty()) {
					...
					if (USTARTS_TI("file://", HrefStr))
						HrefStr = "file:///" + slash_to_yen(get_tkn_m(HrefStr, _T("file:///"), _T("#")));
					else if (!USTARTS_TI("http", HrefStr) && !USTARTS_TI("mailto:", HrefStr) && !USTARTS_TS("//", HrefStr)) {
