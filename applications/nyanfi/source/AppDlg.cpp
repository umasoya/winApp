...
//---------------------------------------------------------------------------
AppWinInf * __fastcall TAppWinList::Get(int Index)
{
 	return (AppWinInf*)(TList::Get(Index));
}
//---------------------------------------------------------------------------
void __fastcall TAppWinList::Put(int Index, AppWinInf *Item)
{
	...
	if (f_cnt) {
		try {
			...
			for (UINT i=0; i<f_cnt; i++) {
				...
				if (atr==faInvalid)		SysErrAbort(ERROR_ACCESS_DENIED);
...
void __fastcall TAppListDlg::UpdateAppList()
{
	...
	while (hWnd) {
		do {
			...
			if (ProcessId==pid && USAME_TI(cnam, "TAppListDlg")) break;
			...
			if (is_new) {
				...
				if (ap->isNyan) {
					...
					//無応答でなければアイコンを取得
					if (!ap->isNoRes) ap->Icon->Handle = (HICON)::SendMessage(hWnd, WM_GETICON, ICON_BIG, 0);
	...
	//通常モード
		...
		if (FindFirst(UAPP_T(CurLaunchPath, "*.*"), faAnyFile, sr)==0) {
			do {
				...
				file_rec *fp = cre_new_file_rec(fnam);  if (!fp) continue;
				//fp->r_name = リンク先, fp->alias = コメント
				if (SortByRem && test_LnkExt(fp->f_ext)) {
					usr_SH->get_LnkInf(fp->f_name, NULL, &fp->r_name, NULL, NULL, NULL, &fp->alias);
				}
				//URL
				else if (SortByRem && USAME_TI(fp->f_ext, ".url")) {
					std::unique_ptr<UsrIniFile> url_file(new UsrIniFile(fnam));
					fp->r_name = url_file->ReadString("InternetShortcut", "URL");
				}
				lst->AddObject(fp->f_name, (TObject*)fp);
			} while(FindNext(sr)==0);
			FindClose(sr);
...
void __fastcall TAppListDlg::UpdateLaunchSttBar()
{
	...
	if (cfp) {
		...
		if (!cfp->is_dir) {
			...
			if (!cfp->r_name.IsEmpty()) msg.cat_sprintf(_T(" → %s"), cfp->r_name.c_str());	//リンク先
		}
	}
		...
		TOwnerDrawState State)
{
	...
	//アイコン
	if (ap->Icon) ::DrawIconEx(cv->Handle, xp, Rect.Top + 2, ap->Icon->Handle, 32, 32, 0, NULL, DI_NORMAL);
	//最小化マーク
...
void __fastcall TAppListDlg::AppListBoxKeyDown(TObject *Sender, WORD &Key, TShiftState Shift)
{
	...
	//アプリケーション情報
	else if (USTARTS_TI("ShowFileInfo", CmdStr) || USAME_TI(CmdStr, "ListFileInfo"))
...
void __fastcall TAppListDlg::RestoreActionExecute(TObject *Sender)
{
	...
	if (ap) {
		if (ap->isNyan) {
			//ヘルプ
			if (USAME_TI(ap->ClassName, "HH Parent")) {
	...
	if (ap) {
		if (ap->isNyan) {
			...
			if (ap->WinHandle==Application->MainForm->Handle)
				ModalResult = mrRetry;
			//ヘルプ
			else if (USAME_TI(ap->ClassName, "HH Parent")) {
...
void __fastcall TAppListDlg::AppInfoActionExecute(TObject *Sender)
{
	...
	if (c_ap) {
		...
		add_PropLine(_T("実行ファイル"),	tmp,	lst.get());
		add_PropLine(_T("実行パス"),		yen_to_slash(ExtractFilePath(c_ap->FileName)), 	lst.get());
		...
		lst->Add(EmptyStr);
		add_PropLine(_T("ウインドウID"),	tmp.sprintf(fmt_08X, ::GetWindowLong(hWnd, GWL_ID)),	lst.get());
		add_PropLine(_T("インスタンス"),	tmp.sprintf(fmt_08X, ::GetWindowLong(hWnd, GWL_HINSTANCE)),	lst.get());
		add_PropLine(_T("プロシージャ"),	tmp.sprintf(fmt_08X, ::GetWindowLong(hWnd, GWL_WNDPROC)),	lst.get());
		add_PropLine(_T("ユーザデータ"),	tmp.sprintf(fmt_08X, ::GetWindowLong(hWnd, GWL_USERDATA)),	lst.get());
		lst->Add(EmptyStr);
		...
		TRect &Rect, TOwnerDrawState State)
{
	...
	if (Index>0) {
		...
		if (UENDS_TS('-', fp->alias)) 			draw_separateLine(cv, Rect, 2);	//コメント
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TAppListDlg::LaunchListBoxKeyDown(TObject *Sender, WORD &Key, TShiftState Shift)
{
	...
	//インクリメンタルサーチモード
	else if (USTARTS_TI("IncSearch", CmdStr)) {
	...
	//ディレクトリの作成
		...
		UnicodeString inpstr = inputbox_ex(LoadUsrMsg(USTR_CreateDir).c_str(), _T("名前"));
		if (!inpstr.IsEmpty()) {
...
void __fastcall TAppListDlg::PropertyItemClick(TObject *Sender)
{
	...
	if (cfp) {
		TListBox *lp = LaunchListBox;
		Mouse->CursorPos = lp->ClientToScreen(Point(12, (lp->ItemIndex - lp->TopIndex) * lp->ItemHeight + lp->ItemHeight/2));
