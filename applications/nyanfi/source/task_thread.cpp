...
//---------------------------------------------------------------------------
TaskConfig * __fastcall TaskConfigList::Get(int Index)
{
	return (TaskConfig*)(TList::Get(Index));
}
//---------------------------------------------------------------------------
void __fastcall TaskConfigList::Put(int Index, TaskConfig *Item)
{
...
__fastcall TTaskThread::TTaskThread(bool CreateSuspended) : TThread(CreateSuspended)
{
	...
	TaskReady  = TaskFinished = false;
	TaskCancel = TaskPause = false;
...
{
	if (!LogErrOnly || must_log || USTARTS_TS("E ", msg) || USTARTS_TS("C ", msg)) {
		UnicodeString s;
...
bool __fastcall TTaskThread::EX_wait_file_ready(UnicodeString fnam)
{
	int wait = 20;	//*** 最大10秒
	while (!TaskCancel
		&& ImgViewThread && ImgViewThread->TaskBusy && SameText(fnam, ImgViewThread->FileName)
		&& wait>0)
	{
		...
		if (DelUseTrash)
			...
			//通常
				...
				for (int i=0; i<fbuf->Count && !TaskCancel; i++) {
					while (TaskPause && !TaskCancel) Sleep(WAIT_INTERVAL);
					SubCount = fbuf->Count - i;
					DEL_core(fbuf->Strings[i], DelUseTrash);
					if (ReqTaskSlow) {
						ReqTaskSlow = false;
						Sleep(SLOW_INTERVAL);
					}
			...
			if (!TaskFinished && TaskCancel)
				...
				if (Config->TaskList->Count>0) {
					...
					if (!CmdName.IsEmpty() && !prmstr.IsEmpty()) {
						...
						switch (idx_of_word_i(_T("CPY|MOV|DEL|CPYDIR|CMPDEL|CVIMG|DLEXIF|EXTCHM|BACKUP|DTIME"), CmdName)) {
