...
void __fastcall TDotNyanDlg::FormShow(TObject *Sender)
{
	...
	Caption = ExcludeTrailingPathDelimiter(get_MiniPathName(UAPP_T(DotNyanName, " の設定"), 420, titFont.get()));
	...
	if (file_exists(inam)) {
		...
		InheritLabel->Caption = UnicodeString().sprintf(_T("[%s] から継承可能"),
									yen_to_slash(get_MiniPathName(ExtractFileDir(inam), 400, Font)).c_str());
	}
}
//---------------------------------------------------------------------------
...
{
	if (Button==mbLeft && ColorComboBox->ItemIndex!=-1)
		UserModule->BeginSpuit(SpuitPanel, SpuitImage);
}
//---------------------------------------------------------------------------
