...
void __fastcall TXmlViewer::FormShow(TObject *Sender)
{
	...
	Caption = UnicodeString().sprintf(_T("%s - XMLビュアー"), yen_to_slash(FileName).c_str());
		...
		TCustomDrawState State, bool &DefaultDraw)
{
	...
	//値
		...
		tmp_cv->Font->Color = is_selfg? col_fgSelItem :
					(USTARTS_TS("http", lbuf) && TRegEx::Match(lbuf, URL_MATCH_PTN).Success)? col_URL : col_fgList;
...
void __fastcall TXmlViewer::XmlTreeViewKeyDown(TObject *Sender, WORD &Key, TShiftState Shift)
{
	...
	else if (USTARTS_TI("IncSearch", cmd_F) || contained_wd_i(KeyStr_Filter, KeyStr))
		FindEdit->SetFocus();
	//閉じる
