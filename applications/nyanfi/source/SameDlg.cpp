...
void __fastcall TSameNameDlg::FormShow(TObject *Sender)
{
	...
	UnicodeString dnam = ExtractFileDir(CurSrcName);
	if (is_match_regex(dnam, _T("^\\\\[^\\\\]"))) dnam = yen_to_slash(dnam);
	tmp.sprintf(_T("元: %s"), dnam.c_str());
	InfoListBox->Items->Add(tmp);
	InfoListBox->Items->Add(EmptyStr);

	dnam = ExtractFileDir(CurDstName);
	if (is_match_regex(dnam, _T("^\\\\[^\\\\]"))) dnam = yen_to_slash(dnam);
	tmp.sprintf(_T("先: %s"), dnam.c_str());
	InfoListBox->Items->Add(tmp);
	InfoListBox->Items->Add(EmptyStr);
