...
void HighlightFile::CheckValues()
{
	...
	for (int i=0; i<SectionList->Count; i++) {
		...
		for (int j=0; j<klist->Count; j++) {
			...
			bool err = false;
			if		(ends_t_i(_T("Ptn"), key)) err = !chk_RegExPtn(vstr);
			else if (ends_t_i(_T("Col"), key)) err = !is_match_regex(vstr, _T("^[0-9a-fA-F]{6}"));
			else if (ends_t_i(_T("File"),key)) err = !file_exists(rel_to_absdir(vstr));
...
//正規表現パターン
//---------------------------------------------------------------------------
UnicodeString HighlightFile::ReadRexExPtn(const _TCHAR *key)
{
	if (CurSection.IsEmpty()) return EmptyStr;
	...
	TColor def)	//デフォルト値	(default = clBlack)
{
	...
	if (is_match_regex(s, _T("^[0-9a-fA-F]{6}"))) {
		col = TColor(
				RGB(("0x" + s.SubString(1,2)).ToIntDef(0),
	...
	bool only_def)			//デフォルトのみを取得	(default = false)
{
	...
	//デフォルト
		...
		else if (test_FileExt(fext, _T(".txt")) && USTARTS_TS("awstats", ExtractFileName(fnam))) {
			ln_lst->Add("#");
		}
	}
	...
	bool only_def)			//デフォルトのみを取得		(default = false)
{
	...
	UnicodeString fext = get_extension(fnam);
	//ユーザ定義
	if (!only_def && GetSection(fnam)) h_ptn = ReadRexExPtn(_T("HeadlinePtn"));
	...
	bool is_h2t)			//HTML→TEXT変換モード
{
	...
	if (is_xml) {
		...
		ret_str.sprintf(_T("%s"),
			...
			_T("Debug|Download|Echo(On|Off|T?)|Else|End(If|Repeat)|Exit|FileExists|FlashWin|Format(DT|")
			_T("DI)?|Goto|Grep(SetMask|Start)|Hint|If[^_]*(_(Break|Continue|Exit||Goto))?|Input|")
			_T("Ins|JumpCommands|List(Buffer|Var)|Match(Buffer|Dir|Ext|Name)|MsgBox[YNC]*|MsgOff|")
			_T("Nop|PlaySound|PopupMenu|(Push|Pop)(Index|TopIndex|Var)|Random|ReadINI|Redraw|Repeat|ReturnCommands|")
			_T("SearchDown[FD]?|SearchUp[FD]?|SendCommands|Set_|Set(Clipboard|CopyMode|FileTime|")
			_T("Index)|SetExe(File|Param|Dir|Opt)|ShellExecute|Sleep|Test|Timer|WaitForTask2?|Warn|WriteINI)_?"));
		sns_case = false;
	}
...
bool GetDefaultHighlight(
	...
	TStringList *hl_lst)	//拡張子対応見出しリスト	(default = NULL)
{
	if (!USTARTS_TS('.', fext)) fext.Insert(".", 1);
