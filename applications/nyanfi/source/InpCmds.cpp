...
void __fastcall TInpCmdsDlg::SetList()
{
	...
	for (int i=0; i<CmdSetList->Count; i++) {
		...
		if (StartsStr(idxStr, lbuf)) ItemList->Add(get_CmdDesc(get_tkn_r(lbuf, ':')));
	}
...
void __fastcall TInpCmdsDlg::CmdsComboBoxKeyDown(TObject *Sender, WORD &Key, TShiftState Shift)
{
	...
	if (equal_ENTER(KeyStr)) {
		...
		if (!cp_down) {
			cp_inp->DroppedDown = true;
			if (cp_inp->Items->Count>0) cp_inp->ItemIndex = 0;
		}
	...
	//状況依存ヘルプ
		...
		UnicodeString kwd = get_tkn(get_CmdStr(CmdsComboBox->Text), ':');
		UnicodeString topic;
		if (kwd.IsEmpty() || SubComboBox->DroppedDown || starts_AT(kwd) || starts_Dollar(kwd)) {
