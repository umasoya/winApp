...
void __fastcall TFindFileDlg::FormShow(TObject *Sender)
{
	...
	//高さ設定
	int hi = OkPanel->Height;
	if (MaskPanel->Visible)		hi += MaskPanel->Height;
	if (KeywordPanel->Visible)	hi += KeywordPanel->Height;
	if (SizePanel->Visible)		hi += SizePanel->Height;
	if (ContPanel->Visible)		hi += ContPanel->Height;
	hi += DatePanel->Height;
	hi += AttrPanel->Height;
	BasicPanel->Height = hi;
	ClientHeight = hi;
...
{
	hasTime = hasRate = hasFrame = hasFormat = hasImage = hasExif = hasProp = hasText = hasIcon = hasCode = hasLCnt = false;
}
...
void __fastcall TFindFileDlg::MaskComboBoxChange(TObject *Sender)
{
	if (ExtPanel->Visible) {
		...
		hasFormat = test_FileExt(fext, _T(".wav"));
		hasImage  = test_FileExt(fext, FEXT_IMAGE + WicFextStr) || SPI->TestFExt(fext);
		hasExif   = test_ExifExt(fext);
		hasText   = is_text || xd2tx_TestExt(fext);
		hasCode   = is_text;
		hasProp   = test_FileExt(fext, FEXT_APPINFO FEXT_VIDEO FEXT_HTML FEXT_FONT FEXT_XD2T_PRP _T(".mp3.png.gif.webp.ico.lnk"));
		hasIcon   = test_FileExt(fext, FEXT_ICONVIEW);
		hasLCnt   = true;
		...
		PropGroupBox->Caption =
			...
			test_IcoExt(fext)?		"アイコン情報(&I)" : "ファイル情報(&I)";
		...
		hasTime = hasRate = hasFrame = hasFormat = hasImage = hasProp = hasText = hasIcon = hasCode = hasLCnt = false;
}
...
void __fastcall TFindFileDlg::AttrCheckBoxClick(TObject *Sender)
{
	if (!AttrRCheckBox->Checked && !AttrACheckBox->Checked &&
		(!AttrHCheckBox->Checked && ShowHideAtr) && (!AttrSCheckBox->Checked && ShowSystemAtr))
...
void __fastcall TFindFileDlg::CondChangeUpdate(TObject *Sender)
{
	...
	ContPanel->Color = (ContRadioGroup->ItemIndex==0)? col_Invalid : scl_BtnFace;
	...
	ExifPanel->Color = (!ExifKwdEdit->Text.IsEmpty() || (!LatLngComboBox->Text.IsEmpty() && !GpsRangeEdit->Text.IsEmpty()))? scl_BtnFace : col_Invalid;
