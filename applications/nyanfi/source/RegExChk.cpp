...
void __fastcall TRegExChecker::FormShow(TObject *Sender)
{
	...
	if (!PatternStr.IsEmpty()) PtnComboBox->Text = PatternStr;
...
{
	if		(ObjComboBox)	 ObjComboBox->Text	  = PtnComboBox->Text;
	else if (ObjLabeledEdit) ObjLabeledEdit->Text = "/" + PtnComboBox->Text + "/";
}
//---------------------------------------------------------------------------
void __fastcall TRegExChecker::PasteActionUpdate(TObject *Sender)
{
	((TAction*)Sender)->Enabled = !PtnComboBox->Text.IsEmpty() && (ObjComboBox || ObjLabeledEdit);
}
...
void __fastcall TRegExChecker::CopyActionExecute(TObject *Sender)
{
	...
	if (((TAction*)Sender)->Tag==1) ptn = "\"" + ReplaceStr(ptn, "\\", "\\\\") + "\"";
