	...
	UnicodeString id)	//追加ID	(default=EmptyStr)
{
	...
	for (unsigned short i=0; i<entn; i++) {
		unsigned short tag   = EXIF_get_data_2(fs, bsw);	//タグ
		unsigned short dtype = EXIF_get_data_2(fs, bsw);	//値のタイプ
		int count = EXIF_get_data_4(fs, bsw);				//値の数
		fs->Read(vbuf, 4);									//値または値へのオフセット
...
void EXIF_format_inf(UnicodeString fext, TStringList *lst)
{
	...
	if (idx!=-1) {
		...
		if (!vstr.IsEmpty()) {
			vstr = get_word_i_idx(_T("?|左上・横(1)|?|?|?|?|左上・縦(6)|?|左下・縦(8)"), vstr.ToIntDef(0));
	...
	try {
		if (isCRW) {
			...
			if (isRaf || isX3f) {
				...
				int bsz = EXIF_get_data_4(fsp, true);	//Boxサイズ
				if (bsz!=24) Abort();
