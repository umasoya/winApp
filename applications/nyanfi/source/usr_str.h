...
//s += "hoge" より s.cat_sprintf(_T("%s"), _T("hoge")) の方が小さくて高速

#define USAME_TI(s1,s2)	same_ut_i(s1, _T(s2))
#define USAME_TS(s1,s2)	same_ut_s(s1, _T(s2))

#define USTARTS_TI(w,s)	starts_t_i(_T(w), s)
#define USTARTS_TS(w,s)	starts_t_s(_T(w), s)

#define UENDS_TI(w,s)	ends_t_i(_T(w), s)
#define UENDS_TS(w,s)	ends_t_s(_T(w), s)
