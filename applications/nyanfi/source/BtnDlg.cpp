...
void __fastcall TToolBtnDlg::RefCmdsBtnClick(TObject *Sender)
{
	...
	//ファイル
	else if (contained_wd_i(_T("ExeMenuFile|FileEdit|FileRun|LoadWorkList|OpenByApp|OpenByWin|TextViewer"), cmd)) {
		...
		if (UserModule->OpenDlgToStr(fnam)) {
			...
			else if (contained_wd_i(_T("ExeMenuFile|LoadWorkList"), cmd))
