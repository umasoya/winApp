...
void __fastcall TImgViewThread::DrawImage()
{
	...
	if (is_viewer) {
		...
		//分割グリッド
		if (is_viewer && ShowGrid) draw_ImgGrid(v_img->Picture->Bitmap->Canvas, ViewBuff, ImgIsWmf);
...
void __fastcall TImgViewThread::Execute()
{
	...
	while (!Terminated) {
		//描画要求
		if (ReqRedraw || ReqReload || ReqChange) {
			if (ReqChange) {
				//ビットマップ(ImgBuff)の変更
				FileName   = FileName2 = EmptyStr;
				Img_f_name = EmptyStr;  Img_f_size = 0;  Img_f_time = 0;  ImgIsWmf = false;
				ImgBuff->Handle = ChgBuff->ReleaseHandle();
				ReqReload = true;
			}
		...
		//ファイルから読み込み
			...
			try {
				...
				if (UseViewBuff) {
					...
					try {
						...
						if (test_FontExt(fext)) {
							UnicodeString fnt_name = get_FontName(Img_f_name);
							if (!fnt_name.IsEmpty()) {
								//フォント名
								cv->TextOut(2, 2, fnt_name);

								bool tmp_flag = false;
								if (Screen->Fonts->IndexOf(fnt_name)==-1)
									tmp_flag = (::AddFontResourceEx(fnam.c_str(), FR_PRIVATE, NULL)>0);

								//文字セット
								BYTE ch_set = DEFAULT_CHARSET;
								if (!tmp_flag) {
									DWORD dwSize = 0;
				...
				//その他のファイル
					...
					MsgStr.USET_T("読込中..."); MsgIsErr = false;
					...
					//MP3
						...
						if (!ID3_GetImage(Img_f_name, ImgBuff)) ReqClear = true;
					}
					//ディレクトリの特大アイコン
					...
					//ZIP
						...
						//アーカイブ内の画像
						int res = 0;
						UnicodeString tmp_name = ExtractInZipImg(Img_f_name);
						if (!tmp_name.IsEmpty()) {
					...
					//画像
					else if (test_FileExt(fext, FEXT_META FEXT_WICSTD + WicFextStr) || SPI->TestFExt(fext))	{
