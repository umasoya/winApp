...
{
	if (!str.IsEmpty()) add_PropLine(tit, str, lst);
}
...
HRESULT __stdcall TDropTarget::Drop(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
	...
	//リンク項目
		...
		if (!fnam.IsEmpty() && !url.IsEmpty()) {
			DroppedList->Add(UnicodeString().sprintf(_T("%s\t%s"), ChangeFileExt(fnam, ".url").c_str(), url.c_str()));
...
//---------------------------------------------------------------------------
drop_target_rec * __fastcall TDropTargetList::Get(int Index)
{
 	return (drop_target_rec*)(TList::Get(Index));
}
//---------------------------------------------------------------------------
void __fastcall TDropTargetList::Put(int Index, drop_target_rec *Item)
{
			...
			if (lst && !idx_str.IsEmpty()) {
				...
				for (int i=0; i<i_lst.Length; i++) {
					...
					if (lst_sw)
						...
						if (UENDS_TS("解像度", tmp) && extract_int(vstr)>0x10000) continue;	//*** 異常値
	...
	UnicodeString *nam, UnicodeString *prm, UnicodeString *fld, int *shw, UnicodeString *rem)
{
	try {
		...
		if (has_id_list) {
			...
			BYTE *bp = (BYTE*)msp->Memory + (sh_ofs + 2);
			_TCHAR pstr[MAX_PATH + 1];
			if (SHGetPathFromIDList((LPCITEMIDLIST)bp, pstr)) pnam = pstr;
		}
		...
		if (ExtractFileDrive(pnam).Pos(':') && USTARTS_TS("\\\\", l_nam))
			l_nam = pnam;
		...
		if (lst) {
			...
			add_PropLine_if(_T("作業場所"),		l_fld, lst);
			...
			if (!l_fnam.IsEmpty()) {
				...
				else add_PropLine_if(_T("エラー"), "リンク先が見つかりません。", lst);
			}
		}
...
UnicodeString UserShell::KnownGuidStrToPath(UnicodeString s)
{
	...
	PWSTR pbuf = NULL;
	if (SUCCEEDED(::SHGetKnownFolderPath(guid, 0, NULL, &pbuf))) {
		ret_str = pbuf;
		CoTaskMemFree(pbuf);
	}
	return ret_str;
