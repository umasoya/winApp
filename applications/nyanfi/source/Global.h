...
#define FEXT_DURATION	FEXT_MCISND FEXT_VIDEO _T(".cda.opus")					//長さを持つ
#define FEXT_ICONVIEW	_T(".exe.dll.bpl.dpl.cpl.ico.cur.ani")					//アイコン抽出可能
#define FEXT_APPINFO	_T(".exe.com.dll.rll.bpl.dpl.cpl.ocx.spi.scr")			//アプリ情報取得可能
...
struct file_rec {
	...
	UnicodeString r_name;		//表示名、リンク先など
	UnicodeString memo;			//マーク項目のメモ(内容 \t 設定日時)  マーク一覧で使用
	...
	bool is_ftp;				//FTP
	bool is_dummy;				//ダミー(空のドライブ、ワークリストのセパレータなど)
	bool selected;				//選択中
