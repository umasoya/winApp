...
__published:	// IDE で管理されるコンポーネント
	...
	void __fastcall TaskPaintBoxMouseUp(TObject *Sender, TMouseButton Button, TShiftState Shift,
          int X, int Y);
	void __fastcall FlScrPanelMouseUp(TObject *Sender, TMouseButton Button, TShiftState Shift,
          int X, int Y);
	void __fastcall SubListBoxMouseDown(TObject *Sender, TMouseButton Button, TShiftState Shift,
          int X, int Y);
	...
	void __fastcall TxtTailListBoxDrawItem(TWinControl *Control, int Index, TRect &Rect,
	...
	void __fastcall ThumbnailGridMouseUp(TObject *Sender, TMouseButton Button, TShiftState Shift,
          int X, int Y);
...
private:	// ユーザー宣言
	...
	void __fastcall ResetIncSeaFilter(TStringList *lst);
	...
	int  __fastcall SelectMask(TStringList *lst, UnicodeString mask);
	...
	void __fastcall AssignFileList(TStringList *lst, int tag, UnicodeString last_nam, int last_idx = -1, int last_top = -1);
	...
	void __fastcall CalcDirSizeCore(file_rec *fp, TStringList *r_lst = NULL,
		__int64 *total_sz = NULL, int *total_f_cnt = NULL, int *total_d_cnt = NULL);
	...
	UnicodeString __fastcall GetCopyFileNames(UnicodeString fmt, bool all_files = false, file_rec *cfp = NULL);
	...
	void __fastcall ShowInpDirPanel();
	...
	TModalResult __fastcall DownloadWorkProgress(UnicodeString url, UnicodeString fnam);
...
public:		// ユーザー宣言
	...
	void __fastcall RefresCurPath(UnicodeString fnam);
	...
	void __fastcall SetSttBarInf(file_rec *fp = NULL);
