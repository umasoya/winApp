		...
		for (int i=0; i<MarkIdxList->Count; i++) {
			...
			if (is_drive_accessible(dnam)) {
				...
				if (ends_PathDlmtr(dnam)) {
					if (dir_exists(dnam)) {
						TStringList *klist = (TStringList*)MarkIdxList->Objects[i];
						int j = 0;
						while (j<klist->Count) {
				...
				//アーカイブ(内部はチェックしない)
				else if (!file_exists(dnam)) EraseSection(dnam);
			}
		}
		UpdateMarkIdxList();
	}
...
//---------------------------------------------------------------------------
bool UsrIniFile::ReadBoolGen(const _TCHAR *key, bool def)
{
...
//ComboBox の項目を読込
//---------------------------------------------------------------------------
void UsrIniFile::LoadComboBoxItems(TComboBox *cp, const _TCHAR *sct, int max_items,
...
//ComboBox の項目を保存
//---------------------------------------------------------------------------
void UsrIniFile::SaveComboBoxItems(TComboBox *cp, const _TCHAR *sct, int max_items)
{
	UnicodeString sct_u = def_if_empty(sct, cp->Name);
	UnicodeString key;
