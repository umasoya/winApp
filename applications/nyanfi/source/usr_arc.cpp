...
UserArcUnit::UserArcUnit(HWND hWnd)
{
	...
	for (int i=0; i<MAX_ARC_DLL; i++) {
		...
		if (fp->hDll) {
			...
			fp->GetArcFileSize		 = (FUNC_ArcGetArcFileSize)GetProcAdr(fp, 		"GetArcFileSize");
			fp->GetArcFileSizeEx	 = (FUNC_ArcGetArcFileSizeEx)GetProcAdr(fp,		"GetArcFileSizeEx");
			fp->GetArcOriginalSize	 = (FUNC_ArcGetArcOriginalSize)GetProcAdr(fp,	"GetArcOriginalSize");
			fp->GetArcOriginalSizeEx = (FUNC_ArcGetArcOriginalSizeEx)GetProcAdr(fp, "GetArcOriginalSizeEx");
			...
			fp->Available = fp->Command && fp->GetVersion && fp->GetRunning && fp->CheckArchive
					...
					&& (fp->GetArcOriginalSize || fp->GetArcOriginalSizeEx)
					&& (fp->GetArcFileSize || fp->GetArcFileSizeEx);
...
//コマンドの実行
//戻り値 : 0 = 成功
// CmdName、CmdLine、ResMsg、ErrMsg を設定
//---------------------------------------------------------------------------
	...
	bool ow_sw)					//確認無しで上書き			(default = false)
{
	...
	//7-zip32.dll が解凍先ディレクトリ名内の連続空白を扱えない問題に対処
...
bool UserArcUnit::FindFirst(UnicodeString mask, INDIVIDUALINFO *inf)
{
	...
	//※uniso32.dll だとUnicodeモードにしてもUTF-8のマスクを受け付けてくれない?
}

//---------------------------------------------------------------------------
bool UserArcUnit::FindFirstEx(UnicodeString dnam, arc_find_inf *inf,
	bool sub_dir)	//サブディレクトリも含む (default = false);
{
	FindDir = dnam;
	...
	if (OpenArc(arc_file)) {
		...
		if (FindFirstEx(EmptyStr, &inf)) {
			do {
				...
				if (fnam.IsEmpty() || lst->IndexOf(fnam)!=-1) continue;
				lst->Add(fnam);
				i_cnt++;
	...
	UnicodeString img_fext)		//対応画像拡張子
{
	...
	try {
		...
		if ((fp->GetAttribute(hCurArc) & FA_ENCRYPTED)!=0) Abort();
		...
		do {
			if (!inf.is_dir) {
				...
				if (test_FileExt(fext, img_fext)) ret = true;
			}
	...
	*is_enc   = false;
	...
	try {
		...
		if (FindFirstEx(EmptyStr, &inf, true)) {
			if (CurType==UARCTYP_ZIP || CurType==UARCTYP_7Z)
			...
			//圧縮率の計算
			__int64 file_size;
			if (fp->GetArcFileSizeEx && fp->GetArcOriginalSizeEx) {
				if (!fp->GetArcFileSizeEx(hCurArc, &file_size))	  Abort();
				if (!fp->GetArcOriginalSizeEx(hCurArc, org_size)) Abort();
			}
			else {
				int size = fp->GetArcFileSize(hCurArc);	if (size==-1) size = 0;	//***
				file_size = size;
				size = fp->GetArcOriginalSize(hCurArc);	if (size==-1) size = 0;	//***
				*org_size = size;
			}
			if (!is_rar_part) *ratio = (*org_size>0 && file_size>0)? 100.0 * file_size/(*org_size): 0.0;
		}
	}
	catch (EAbort &e) {
		ret = false;
	...
	//RARで取得できなかった場合 -l コマンドで
	if (*org_size==0 && CurType==UARCTYP_RAR) {
		try {
			UnicodeString cmd = "-l " + add_quot_if_spc(arc_file) + " *";
			if (ExeCommand(CurType, fp, cmd, true)!=0)	Abort();

			std::unique_ptr<TStringList> r_lst(new TStringList());
			r_lst->Text = ResMsg;
			if (r_lst->Count==0) Abort();
			UnicodeString lbuf = get_tkn_r(r_lst->Strings[r_lst->Count - 1], _T("File"));
			if (lbuf.IsEmpty())  Abort();
			//サイズ
			if (lbuf[1]=='s') lbuf.Delete(1, 1);
			*org_size = StrToInt64Def(get_tkn(Trim(lbuf), ' '), 0);
			//圧縮率(分割されている場合は取得しない)
			if (!is_rar_part) {
				int p = pos_r(_T(" "), lbuf);	if (p==0)			  Abort();
				lbuf.Delete(1, p);				if (!contains_s(lbuf, _T('%'))) Abort();
				lbuf = get_tkn(lbuf, '%');		if (lbuf.IsEmpty())	  Abort();
				*ratio = StrToFloat(lbuf);
			}
		}
		catch (EAbort &e) {
			*org_size = 0;
