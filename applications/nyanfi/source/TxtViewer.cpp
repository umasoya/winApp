	...
	TPanel *colref)			//カラー表示
{
	...
	isContinue	= false;
	ShowRuby	= true;
	isAozora	= isLog = isClip = isIniFmt = isAwstats = false;
...
void __fastcall TTxtViewer::SetMetric(bool set_hi)
{
	...
	if (TabLength==0) TabLength = test_FileExt(get_extension(FileName), FExtViewTab4)? 4 : 8;
...
void __fastcall TTxtViewer::FormatFixed(TStringList *txt_lst)
{
	...
	//１行目
	bool is_tsv = contains_s(txt_lst->Strings[0], _T('\t'));
	...
	int lno)	//行番号 0 = 現在行		(default = 1)
{
	...
	if (!isBinary && TxtBufList->Count>0)
		is_xml = (USTARTS_TI("<?xml ", TxtBufList->Strings[0]) || test_FileExt(fext, FEXT_XML));

	//青空文庫か？
	isAozora = ChkAozora && test_FileExt(fext, _T(".txt")) && is_match_regex(TxtBufList->Text, _T("［＃.*?］"));

	isIniFmt = isAwstats = false;
	if (!isBinary && !isLog && !is_xml && !isAozora) {
		...
		//AWStatsデーか？
		if (test_FileExt(fext, _T(".txt")) && USTARTS_TS("awstats", ExtractFileName(FileName))) {
			isAwstats = USTARTS_TS("AWSTATS DATA FILE", TxtBufList->Text);
		}
	}
	...
	if (!isBinary) {
		if (usr_hl) {
			...
			ReservedPtn   = UserHighlight->ReadRexExPtn(_T("ReservedPtn"));
			ReservedCase  = UserHighlight->ReadKeyBool( _T("ReservedCase"), true);
			NumericPtn	  = UserHighlight->ReadRexExPtn(_T("NumericPtn"));
			...
			UsrKeyword	  = UserHighlight->ReadRexExPtn(_T("KeywordPtn"));
			UsrKeywdCase  = UserHighlight->ReadKeyBool( _T("KeywordCase"), true);
			UsrKeyword2   = UserHighlight->ReadRexExPtn(_T("KeywordPtn2"));
			UsrKeywdCase2 = UserHighlight->ReadKeyBool( _T("KeywordCase2"), true);

			//見出し行
			UnicodeString lbuf = UserHighlight->ReadRexExPtn(_T("HeadlinePtn"));
	...
	//テキスト
		...
		if (test_HtmlExt(fext) && isHtm2Txt) {
			...
			htmcnv->Convert();
			for (int i=0; i<htmcnv->TxtBuf->Count; i++) htmcnv->TxtBuf->Objects[i] = (TObject*)i;
			TxtBufList2->Assign(htmcnv->TxtBuf);
			txt_buf->Assign(htmcnv->TxtBuf);
		}
		//固定長表示
		else if (test_FileExt(fext, FEXT_CSV)) {
			isTSV = (txt_buf->Count>0 && contains_s(txt_buf->Strings[0], _T('\t')));
			...
			if (RemLnList->Count>0 || RemBgnList->Count>0) {
				...
				if (RemLnList->Count>0) {
					...
					if (USAME_TS(RemLnList->Strings[0], "$BAT")) {
						if (USTARTS_TI("REM", tbuf) || USTARTS_TI("@REM",tbuf) || USTARTS_TS("::", tbuf)) {
							RemPos0 = 1;  RemPos1 = lbuf_len;
						}
					}
					else if (USAME_TS(RemLnList->Strings[0], "$QBT")) {
						if (USTARTS_TI("REM", tbuf) || USTARTS_TS(';', tbuf)) {
							RemPos0 = 1;  RemPos1 = lbuf_len;
						}
					}
	...
	int sort_mode)		//ソートモード 0:解除/ 1:昇順/ -1:降順
{
	...
	if (lst) {
		...
		for (int i=0; i<TxtBufList->Count; i++) TxtBufList->Objects[i] = (TObject*)i;
	}
	else {
		//ソート
		cursor_HourGlass();
		...
		if (SortMode==0)
			...
			//通常
			else TxtBufList->CustomSort((SortMode==1)? comp_AscendOrder : comp_DescendOrder);
		}
		cursor_Default();
	}
	...
	int max_x)			//最大幅
{
	...
	while (!s.IsEmpty()) {
		...
		if (ctrl_p>0) {
			...
			//タブ文字
			if (ctrl_ch==_T('\t')) {
				cv->Font->Color = color_TAB;
				UnicodeString tab_str = ShowTAB? L"\u21c0" : L" ";
				TRect rc = Rect(x, y, x + HchWidth, f_h);
				cv->TextRect(rc, x, y, tab_str);	x += HchWidth;
				int dt = (x - TopXpos) % TabWidth;
				if (dt>0) {
					int x1 = x + TabWidth - dt;
					while (x<x1) {
						cv->TextOut(x, y, " ");	x += HchWidth;
						if (x>=max_x) break;
					}
				}
				s.Delete(1, ctrl_p + tab_str.Length() - 1);
				cv->Font->Color = fgcol;
			}
			//その他の制御文字
			else {
			...
			if (has_cr) {
				//改行
				cv->Font->Color = color_CR;
				UnicodeString cr_s = ShowCR? L"\u21b5" : L" ";
				TRect rc = Rect(x, y, x + get_TextWidth(cv, cr_s, IsIrregFont), f_h);
				cv->TextRect(rc, x, y, cr_s);
				cv->Font->Color = fgcol;
			}
			s = EmptyStr;
		}
...
void __fastcall TTxtViewer::PaintText()
{
	...
	tmp_bmp->SetSize(ViewBox->ClientWidth, LineHeight);
	...
	for (int i=top_idx; i<=btm_idx; i++, v_yp+=LineHeight) {
		...
		if (i<MaxDispLine) {
			...
			if (i == MaxDispLine-1) {
				...
				//固定長表示の縦罫線
				if (isFixedLen && lp->LineIdx==0 && color_bdrFixed!=clNone) {
					int v_xp = TopXpos;
					for (int i=0; i<FixWdList.Length-1; i++) {
						v_xp += (FixWdList[i] + 1) * HchWidth;
...
void __fastcall TTxtViewer::onRulerPaint(TObject *Sender)
{
	...
	//折り返し位置
	if (!isFitWin) {
		xp = TopXpos + (isBinary? MaxHchX : ViewFoldWidth) * HchWidth - 1;
...
void __fastcall TTxtViewer::SetSttInf(UnicodeString msg)
{
	...
	if (SttHeader) {
		...
		sttstr = (!isXDoc2Txt && !test_FileExt(fext, _T(".rtf")) && isText)?
						get_NameOfCodePage(TxtBufList->Encoding->CodePage) : UnicodeString("----");
		if (HasBOM) sttstr.UCAT_T(" BOM付");
		...
		else if (test_FileExt(fext, FEXT_CSV))			sttstr.sprintf(_T("%s"), isFixedLen? _T("FIXED") : isTSV? _T("TSV") : _T("CSV"));
		...
		//行桁
			...
			if (CsvCol>=0) sttstr.cat_sprintf(_T(" %u列"), CsvCol + 1);
			//選択状態
			if (SelStart!=SelEnd) {
		...
		if (isText && TxtColorHint && ColorPanel && SelStart==SelEnd) {
			int xp;
			UnicodeString colstr = GetCurWord(false, "(#[0-9a-fA-F]{3}\\b)|(#[0-9a-fA-F]{6}\\b)|(0x|\\$)[0-9a-fA-F]{6,8}", &xp);
	...
	if (isText && CsvRecForm->Visible) {
		line_rec *lp = get_LineRec(CurPos.y);
		if (lp->LineIdx==0 && lp->LineNo>0) CsvRecForm->UpdateRecord(TxtBufList, lp->LineNo - 1);
	}
...
void __fastcall TTxtViewer::onRulerDblClick(TObject *Sender)
{
	...
	int wd = (p.x - TopXpos) /HchWidth;
	isFitWin = (wd==0);
	if (!isFitWin) ViewFoldWidth = wd;
	...
	bool append)	//true = 追加コピー (default = false)
{
	...
	if (!sel_str.IsEmpty()) {
		if (append) sel_str.Insert(Clipboard()->AsText, 1);	//追加
...
UnicodeString __fastcall TTxtViewer::GetCurImgFile()
{
	...
	if (test_FileExt(fext, FEXT_META FEXT_WICSTD + WicFextStr) || SPI->TestFExt(fext)) return fnam;
	return EmptyStr;
}
...
//指定行番号に移動
//---------------------------------------------------------------------------
bool __fastcall TTxtViewer::ToLine(int lno)
{
...
void __fastcall TTxtViewer::JumpLine(UnicodeString ln_str)
{
	try {
		...
		//バイナリ
		if (isBinary) {
			msg.sprintf(_T("0～%s (%llu)"), get_AddrStr(BinFileSize - 1).c_str(), BinFileSize - 1);
			if (ln_str.IsEmpty()) ln_str = Trim(inputbox_ex(_T("指定アドレスに移動"), msg.c_str()));
		...
		//テキスト
			...
			msg.sprintf(_T("行番号(1～%u)"), MaxLine);
			if (ln_str.IsEmpty()) ln_str = Trim(inputbox_ex(_T("指定行番号に移動"), msg.c_str()));
			if (ln_str.IsEmpty())  throw EAbort(EmptyStr);	//キャンセル
...
bool __fastcall TTxtViewer::ExeCommand(const _TCHAR *t_cmd, UnicodeString prm)
{
	...
	if (isExtWnd) {
		if (contained_wd_i(_T("BitmapView|CharInfo|CsvRecord|Inspector|ImgPreview"), cmd)) return false;
	}
	...
	//-------------------------------
		...
		//Web で検索
			...
			if (!kw.IsEmpty()) {
				UnicodeString urlstr = get_EncodedUrl(ReplaceStr(WebSeaUrl, "\\S", kw));
				if (urlstr.IsEmpty())
					GlobalErrMsg = LoadUsrMsg(USTR_NotFound, _T("URL"));
				else
					Execute_ex(urlstr);
...
bool __fastcall TTxtViewer::IsCmdAvailable(UnicodeString cmd)
{
	...
	if (isText) {
		return(contained_wd_i(
			_T("CharInfo|CsvRecord|FindLinkDown|FindLinkUp|FixedLen|HelpCurWord|HtmlToText|ImgPreview|")
...
bool __fastcall TTxtViewer::IsCmdInhibited(UnicodeString cmd)
{
	...
	if (isBinary) {
		return(contained_wd_i(
			_T("ChangeCodePage|CharInfo|CsvRecord|FindLinkDown|FindLinkUp|FixedLen|HelpCurWord|HtmlToText|")
			_T("ImgPreview|OpenURL|SetUserDefStr|ShowRuby|Sort|WebSearch|")
