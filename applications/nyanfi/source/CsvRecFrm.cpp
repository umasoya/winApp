...
//内容の更新
//---------------------------------------------------------------------------
void __fastcall TCsvRecForm::UpdateRecord(TStringList *lst, int idx)
{
	if (lst) DataList = lst;
	TStringGrid *gp = RecordGrid;
	...
	try {
		...
		RecIndex = (idx>=0 && idx<DataList->Count)? idx : 0;
		DatIndex = DataList->IndexOfObject((TObject*)idx);
		...
		if (org_row<gp->RowCount) gp->Row = org_row;
