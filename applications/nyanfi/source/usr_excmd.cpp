...
ExeCmdsList::ExeCmdsList(UnicodeString cmds)
{
	...
	if (remove_top_AT(cmds)) {
		...
		if (!loaded) {
			...
			while (i<f_buf->Count) {
				UnicodeString lbuf = Trim(f_buf->Strings[i]);
				if (UENDS_TI(" #noreload", lbuf)) {
					CmdRec->noreload = true;
					f_buf->Delete(i);
				}
				else if (lbuf.IsEmpty() || USTARTS_TS(';', lbuf))
					f_buf->Delete(i);
				else i++;
			}
			cmds = f_buf->Text;
		...
		//偽
			...
			if (!EndOfCmds && prm.IsEmpty()) {
				...
				while (!EndOfCmds) {
					...
					if (USTARTS_TI("If", lbuf) && !contains_s(lbuf, _T('_')))
						if_lvl++;
					else {
						if (if_lvl==0) {
...
void XCMD_Initialize(UnicodeString &opt)
{
	...
	while (i<XCMD_VarList->Count) {
		if (USTARTS_TS("..", XCMD_VarList->Strings[i])) i++; else XCMD_VarList->Delete(i);
	}
...
void XCMD_upd_Var()
{
	...
	case SCMD_TVIEW:
		if (TxtViewer->isReady) {
			...
			if (!TxtViewer->isBinary && TxtViewer->TxtBufList->Encoding)
	...
	//ソートモード
	XCMD_set_Var(_T("SortMode"), get_word_i_idx(_T("F|E|D|S|A|L"), CurSortMode()));
	...
	if (!XCMD_prm.IsEmpty()) {
		//変数の解決
		if (contains_s(XCMD_prm, _T('%'))) {
			for (int i=0; i<XCMD_VarList->Count; i++)
				XCMD_prm = ReplaceText(XCMD_prm, "%" + XCMD_VarList->Names[i] + "%", XCMD_VarList->ValueFromIndex[i]);
		}
		//環境変数の解決
	...
	UnicodeString err)	//エラー	(default = EmptyStr)
{
	...
	if (XCMD_xlp && !XCMD_xlp->FileName.IsEmpty()) {
		...
		if (cp) {
			...
			i_lst->Add(ExtractFileName(XCMD_xlp->FileName));
			i_lst->Add(yen_to_slash(ExtractFilePath(XCMD_xlp->FileName)));
			i_lst->Add(get_FileInfStr(XCMD_xlp->FileName));
			...
			if (XCMD_VarList->Count>0) {
				...
				DebugForm->ReferListBox->Items->Assign(i_lst.get());
			}
		}
	}
}
	...
	if (remove_top_text(XCMD_cmd, _T("If"))) {
		...
		for (int i=0; i<cnd_lst.Length; i++) {
			...
			switch (idx_of_word_i(
				...
				_T("Shift|Ctrl|Alt|Grep|Connected|Primary|Duplicated|FTP|TimeOut"),			//30～38
				cnd))
			{
...
void XCMD_Input(UnicodeString prm)
{
	...
	if (USAME_TI(prm, "Buffer")) XCMD_BufChanged = true;
}
...
void XCMD_AppendBuffer(UnicodeString prm)
{
	try {
		...
		std::unique_ptr<TStringList> f_buf(new TStringList());
		f_buf->Text = XCMD_GetBuffer();
		TFile::AppendAllText(prm, f_buf->Text, enc.get());
	}
	catch (...) {
		UserAbort(USTR_FaildSave);
	}
