...
//対応拡張子
#define FEXT_ZIP	_T(".zip.apk.jar.cbz.epub.mobi.docx.dotx.pptx.xlsx.xltx.odt.ods.odp.odg.odc.odf.odi.odm.odb")
...
#define FEXT_ZIPIMG	_T(".zip.cbz.epub")		//画像を持つ可能性のあるZIP
...
struct arc_func {
	...
	FUNC_SevenZipExists7zdll		Exists7zdll;	//7z.dll対応版7-zip32.dll独自API
...
public:
	...
	UnicodeString CmdLine;		//実行されたコマンドライン
	...
	bool CheckArc(UnicodeString arc_file);
	...
	bool HasZipImg(UnicodeString arc_file, UnicodeString img_fext);

	bool GetArcInfo(UnicodeString arc_file, int *f_cnt, __int64 *org_size, float *ratio, bool *is_enc, TStringList *lst = NULL);
